...
class MapControlViewController: UIViewController, BMKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        // 在导航栏上添加“截图”按钮
        let screenshotBarButton = UIBarButtonItem(title: "截图", style: .Plain, target: self, action: Selector("screenshoot"))
        ...
        constraints.append(mapView.topAnchor.constraintEqualToAnchor(lbl_text.bottomAnchor, constant: 8))
    ...
    // 截图并保存到相册当中
    func screenshoot() {
        let image = mapView.takeSnapshot()
    ...
    func mapView(mapView: BMKMapView!, regionDidChangeAnimated animated: Bool) {
        print("当前地图区域发生了变化(x = \(mapView.visibleMapRect.origin.x), y = \(mapView.visibleMapRect.origin.y), ")
        print("width = \(mapView.visibleMapRect.size.width), height = \(mapView.visibleMapRect.size.height). )")
        print("ZoomLevel = \(mapView.zoomLevel), RotateAngle = \(mapView.rotation), OverlookAngle = \(mapView.overlooking)")
    }
    ...
    @IBAction func TextfieldValueChange(sender: UITextField) {
        guard let senderValue = sender.text, let value = Float(senderValue) else { return }
        switch sender.tag {
        case 0:
